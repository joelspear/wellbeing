...
function generateCSV(data) {
  const headers = [
    'Teacher', 'Email', 'Date', 'Overall', 'Stress', 'Anxiety', 'Depression',
    'Flagged', 'Q1 Mood', 'Q2 Balance', 'Q3 Support', 'Q4 Workload',
    'Q5 Anxiety', 'Q6 Hope', 'Q7 Sleep', 'Q8 Connection', 'Q9 Confidence',
    'Q10 Open Text',
  ];

  const rows = data.map((r) => [
    r.teacher_name, r.teacher_email, r.date, r.overall, r.stress,
    r.anxiety, r.depression, r.is_flagged ? 'Yes' : 'No',
...
export default function PrincipalResponses() {
      ...
      try {
        ...
        if (data && data.length > 0) {
          setResponses(data.map((r) => ({
            ...
            q7: r.q7_sleep, q8: r.q8_connection, q9: r.q9_confidence,
            q10: r.q10_open_text || '',
          })));
        } else {
  ...
  return (
    <div className="p-responses">
      <h1 className="p-responses-title">Check-in Responses</h1>

      <div className="p-responses-controls">
        <div className="p-responses-filters">
          <div className="p-responses-date-group">
            <label className="p-responses-label" htmlFor="date-from">From</label>
            <input
              id="date-from"
              type="date"
              className="input-field p-responses-date-input"
              value={dateFrom}
              onChange={(e) => setDateFrom(e.target.value)}
            />
          </div>
          <div className="p-responses-date-group">
            <label className="p-responses-label" htmlFor="date-to">To</label>
            <input
              id="date-to"
              type="date"
              className="input-field p-responses-date-input"
          ...
          <div className="p-responses-date-group">
            <label className="p-responses-label" htmlFor="sort-by">Sort by</label>
            <select
              id="sort-by"
              className="input-field p-responses-select"
              value={sortBy}
        ...
        <button
          type="button"
          className="btn-secondary p-responses-export"
      ...
      {loading ? (
        <div className="p-responses-loading">Loading responses...</div>
      ) : sorted.length === 0 ? (
        <div className="p-responses-empty card">
          <p>No responses found for the selected date range.</p>
        </div>
      ) : (
        <div className="p-responses-table-wrap card">
          <table className="data-table p-responses-table">
            <thead>
              <tr>
                <th>Teacher</th>
                <th>Date</th>
                ...
                return (
                  <Fragment key={r.id}>
                    <tr
                      className={
                        'p-responses-row' +
                        (r.is_flagged ? ' p-responses-row--flagged' : '') +
                        (isExpanded ? ' p-responses-row--expanded' : '')
                      }
                      onClick={() => toggleExpand(r.id)}
                    >
                      <td className="p-responses-teacher-cell">
                        <span className="p-responses-teacher-name">{r.teacher_name}</span>
                      </td>
                      ...
                      <td>
                        <span className={`score-${stressTier}`}>{r.stress.toFixed(2)}</span>
                      </td>
                      <td>
                        <span className={`score-${anxietyTier}`}>{r.anxiety.toFixed(2)}</span>
                      </td>
                      <td>
                        <span className={`score-${depressionTier}`}>{r.depression.toFixed(2)}</span>
                      </td>
                      <td>
                        {r.is_flagged ? (
                          <span className="badge badge-red">Flagged</span>
                        ) : (
                          <span className="p-responses-ok">--</span>
                        )}
                      </td>
                    </tr>
                    {isExpanded && (
                      <tr className="p-responses-detail-row">
                        <td colSpan={7}>
                          <div className="p-responses-detail">
                            <div className="p-responses-detail-header">
                              <h3 className="p-responses-detail-title">
                                Individual Question Scores
                              </h3>
                              <span className="p-responses-detail-email">
                                {r.teacher_email}
                              </span>
                            </div>
                            <div className="p-responses-detail-grid">
                              {questionLabels.map(({ key, label }) => {
                                const val = r[key];
                                const tier = getScoreTier(val);
                                return (
                                  <div className="p-responses-detail-item" key={key}>
                                    <span className="p-responses-detail-label">{label}</span>
                                    <span className={`p-responses-detail-value score-${tier}`}>
                                      {val}/5
                                    </span>
                                  </div>
                                );
                            ...
                            {r.q10 && (
                              <div className="p-responses-detail-text">
                                <span className="p-responses-detail-label">Q10 - Open text</span>
                                <p className="p-responses-detail-comment">{r.q10}</p>
                              </div>
                            )}
                          </div>
                        </td>
